...
export default function AttendForm({
    ...
    const submit: FormEventHandler = (e) => {
        ...
        setTimeout(() => {
            post(`/daily-briefings/attend/${token}`, {
                onSuccess: () => {
                    setCaptchaAnswer('');
                    clearSignature();
                }
    ...
    const tglFormat = new Date(briefing.tanggal).toLocaleDateString('id-ID', { day: '2-digit', month: '2-digit', year: 'numeric' });
    const sortedAttendees = [...(briefing.attendees || [])].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());

    return (
        <>
            <Head title={`Daftar Hadir - ${briefing.judul}`} />
